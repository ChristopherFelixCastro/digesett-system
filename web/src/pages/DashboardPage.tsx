...
export default function DashboardPage() {
  ...
  useEffect(() => {
    if (auth.cedula) setCedulaBusqueda(auth.cedula);
    cargarActas(auth.cedula ?? undefined);
  }, []);

  const cargarActas = async (cedula?: string) => {
    setLoading(true);
    setError(null);
    setOffline(false);
    try {
      const params = cedula ? { cedula } : {};
      const { data } = await axiosClient.get('/api/v1/actas', { params });
  ...
  const buscarOtraPersona = () => {
    const cedula = cedulaBusqueda.replace(/\D/g, '');
    if (!cedula) return;
    cargarActas(cedula);
  };
  ...
  return (
    <div style={{ maxWidth: '900px', margin: '0 auto', padding: '2rem 1rem' }}>
      ...
      <div style={{
        ...
        <div>
          ...
          <p style={{ color: '#64748B', fontSize: '.88rem', marginTop: '.25rem' }}>
            Bienvenido, <strong>{auth.nombre || auth.rol}</strong> — Gestiona tus multas e impugnaciones
          </p>
        </div>
        <button
      ...
      <div style={{
        ...
        <label style={{
          fontSize: '.82rem', fontWeight: 600, color: '#374151',
          whiteSpace: 'nowrap'
        }}>
        ...
        <input
          ...
          onKeyDown={e => e.key === 'Enter' && buscarOtraPersona()}
      ...
      {offline && <ErrorMessage offline onRetry={() => cargarActas(cedulaBusqueda.replace(/\D/g, '') || auth.cedula || undefined)} />}
      {error   && <ErrorMessage status={error} onRetry={() => cargarActas(cedulaBusqueda.replace(/\D/g, '') || auth.cedula || undefined)} />}

      {/* Lista de actas */}
      {!loading && !error && !offline && actas.length === 0 && (
        <div style={{
      ...
      {!loading && !error && !offline && actas.map(acta => (
